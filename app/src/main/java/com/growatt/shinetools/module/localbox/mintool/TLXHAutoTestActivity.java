    ...
    Handler mHandlerWriteSN = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (what) {
                ...
                case SocketClientUtil.SOCKET_SEND:
                    BtnDelayUtil.sendMessageWrite(this);
                    sendBytesSN = SocketClientUtil.sendMsgToServer(mClientUtilWriterSN, funs[0]);
                    LogUtil.i("发送写入：" + SocketClientUtil.bytesToHexString(sendBytesSN));
                    break;
                //接收字节数组
                case SocketClientUtil.SOCKET_RECEIVE_BYTES:
                    BtnDelayUtil.receiveMessage(this);
                    try {
                        ...
                        if (isCheck){
                            ...
                            if (deviceBeen.getNewModel() == 0) {
                                ...
                                mReportBean.setModel(MaxUtil.getDeviceModelNew4(deviceBeen.getNewModel()));
                            }
                        }
                        LogUtil.i("接收写入：" + SocketClientUtil.bytesToHexString(bytes));
                    } catch (Exception e) {
...
////                        sendMsg(mClientUtilRead, funs[count]);
//                        byte[] sendBytesR = SocketClientUtil.sendMsgToServer(mClientUtilRead, funs[count]);
...
//                                    mRegistBean.setInput276(MaxWifiParseUtil.obtainValueOne(MaxWifiParseUtil.subBytes125(bs,276,0,1)));
//                                    mRegistBean.setInput277(MaxWifiParseUtil.obtainValueOne(MaxWifiParseUtil.subBytes125(bs,277,0,1)));
//                                    mRegistBean.setInput278(MaxWifiParseUtil.obtainValueOne(MaxWifiParseUtil.subBytes125(bs,278,0,1)));
//                                    mRegistBean.setInput279(MaxWifiParseUtil.obtainValueOne(MaxWifiParseUtil.subBytes125(bs,279,0,1)));
//                                    mRegistBean.setInput280(MaxWifiParseUtil.obtainValueOne(MaxWifiParseUtil.subBytes125(bs,280,0,1)));
//                                    mRegistBean.setInput281(MaxWifiParseUtil.obtainValueOne(MaxWifiParseUtil.subBytes125(bs,281,0,1)));
//                                    mRegistBean.setInput282(MaxWifiParseUtil.obtainValueOne(MaxWifiParseUtil.subBytes125(bs,282,0,1)));
//                                    mRegistBean.setInput283(MaxWifiParseUtil.obtainValueOne(MaxWifiParseUtil.subBytes125(bs,283,0,1)));
//                                    updateUI();
//                                    break;
//                            }
//                        }
...
//                                mReportBean.setFinishTime(MyUtils.getFormatDate("HH:mm:ss", new Date()));
//                                OssUtils.circlerDialog(TLXHAutoTestActivity.this, note, -1, false, () -> {
//                                    EventBus.getDefault().postSticky(mReportBean);
//                                    jumpTo(TLXHToolAutoTestReportActivity.class, false);
//                                });
//                                //关闭连接
...
//                            }
//                        }
//                        if (isFirst && count < funs.length - 1) {
//                            count++;
//                            this.sendEmptyMessage(SocketClientUtil.SOCKET_SEND);
//                        } else {
//                            count = 1;
//                            //先判断，再次读取
//                            this.sendEmptyMessageDelayed(SocketClientUtil.SOCKET_SEND,500);
//                        }
...
//                    try {
////                        MyControl.showJumpWifiSet(TLXHAutoTestActivity.this);
    ...
    Handler mHandlerRead = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (what) {
                ...
                case SocketClientUtil.SOCKET_SEND:
                    BtnDelayUtil.sendMessage(this);
                    byte[] sendBytesR = SocketClientUtil.sendMsgToServer(mClientUtilRead, funs[1]);
                    LogUtil.i("发送读取：" + SocketClientUtil.bytesToHexString(sendBytesR));
                    break;
                //接收字节数组
                case SocketClientUtil.SOCKET_RECEIVE_BYTES:
