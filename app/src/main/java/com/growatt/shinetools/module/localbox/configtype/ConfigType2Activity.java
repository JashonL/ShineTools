 ...
 * 2个寄存器分别设置
 */
public class ConfigType2Activity extends BaseActivity implements Toolbar.OnMenuItemClickListener{
    ...
    protected int getContentView() {
        mContext = this;
        return R.layout.activity_config_type2;
    }
...
//                , {"检查固件1(233)", "检查固件2(234)"}
                //参数设置
                {getString(R.string.android_key1021), getString(R.string.android_key1021)},
                {String.format("%s", getString(R.string.android_key824)), String.format("%s", getString(R.string.android_key822))},
                {String.format("%s", getString(R.string.android_key824)), String.format("%s", getString(R.string.android_key822))},
                {String.format("%s", getString(R.string.android_key824)), String.format("%s", getString(R.string.android_key822))},
                {String.format("%s", getString(R.string.android_key824)), String.format("%s", getString(R.string.android_key822))},
                {String.format("%s", getString(R.string.android_key824)), String.format("%s", getString(R.string.android_key822))},
                {String.format("%s", getString(R.string.android_key824)), String.format("%s", getString(R.string.android_key822))},
                {getString(R.string.android_key824), getString(R.string.android_key822)},
                {getString(R.string.android_key824), getString(R.string.android_key822)},
...
//                , {"AC频率限制时间3低(78)", "AC频率限制时间3高(79)"}
        ...
        tvTitle1.setText(String.format("%s%s", titles[mType][0], registers[mType][0]));
        tvTitle2.setText(String.format("%s%s", titles[mType][1], registers[mType][1]));
        //需要设置的内容
        nowSet = new int[][]{
                {6, funs[mType][1], -1}
                , {6, funs[mType][2], -1}
                ...
                case SocketClientUtil.SOCKET_RECEIVE_BYTES:
                    ...
                    try {
                        ...
                        if (isCheck) {
                            ...
                            //更新ui
                            try {
                                tvContent1.setText(readStr + ":" + getReadValueReal(0, value0));
                            } catch (Exception e) {
                                e.printStackTrace();
                                tvContent1.setText(readStr + ":" + value0);
                            }
                            try {
                                tvContent2.setText(readStr + ":" + getReadValueReal(1, value1));
                            } catch (Exception e) {
                                e.printStackTrace();
                                tvContent2.setText(readStr + ":" + value1);
                            }
