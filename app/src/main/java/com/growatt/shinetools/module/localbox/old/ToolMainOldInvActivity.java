...
public class ToolMainOldInvActivity extends DemoBase implements View.OnClickListener, BaseQuickAdapter.OnItemClickListener {
...
//    int[][] funs = {{3, 0, 124}, {4, 0, 124}};
    int[][] funs = {{3, 0, 44}, {3, 45, 89},{3,180,224},{4, 0, 44},{4,45,89},{4,90,134}};
    //读机器型号功能码
    ...
    //自动刷新功能码
    int[][] autoFun = {{4, 0, 44},{4,45,89},{4,90,134}};
    int autoCount = 0;
    //开启自动刷新
...
//        initRecyclerView();
    ...
    private void initString() {
        ...
        c6Title1 = new String[]{
//                getString(R.string.m305并网倒计时),
                getString(R.string.m306功率百分比),
//                "ISO", getString(R.string.m307内部环境温度),  getString(R.string.m308Boost温度),
...
//                getString(R.string.m313机器型号),
                ...
                getString(R.string.inverterdevicedata_property)
//                ,  getString(R.string.m316固件内部版本)
...
//        tvAdvanceSet = (TextView) header1.findViewById(R.id.tvAdvanceSet);
        ...
        String[] mConTitles = new String[]{
                getString(R.string.快速设置), getString(R.string.m283设置配置), getString(R.string.m284参数设置),
                 getString(R.string.m286高级设置)
//                "设置配置","参数设置","智能检测","高级设置"
        };
        int[] mConImgId = new int[]{
                R.drawable.qiuckset,  R.drawable.max_set, R.drawable.max_parameter, R.drawable.max_advance_set
        };
        ...
        if (END_USER == ShineToosApplication.getContext().getUser_type()){
            ...
            mConImgId = new int[]{
                    R.drawable.qiuckset,  R.drawable.max_set, R.drawable.max_parameter
            };
        }
    ...
    private void initContent1() {
        ...
        mC1Adapter = new MaxContentAdapter(R.layout.item_grid_textview_max_big, mC1List);
...
//        adapter.setNewData(newList);
    ...
    private void initContent4() {
        ...
        mC4Adapter = new MaxContentAdapter(R.layout.item_grid_textview_max_big_3col, mC4List);
...
//                warnCodeStr = String.format("%d(%02d)",warmCode,warmCodeSecond<100?warmCodeSecond:warmCodeSecond%100);
        ...
        String[] datasAbout = new String[c5Title1.length];
...
//        }
        //grid:内部参数
        String[] datasParams = new String[c6Title1.length];
//        datasParams[0] = deviceBeen.getLastTime();
        datasParams[0] = deviceBeen.getRealOPowerPercent();
...
//        datasParams[4] = deviceBeen.getBoostTemp();
        datasParams[1] = deviceBeen.getDeviceTemp();
        datasParams[2] = deviceBeen.getpBusV();
        datasParams[3] = deviceBeen.getnBusV();
        datasParams[4] = deviceBeen.getPidErrCode();
        //处理pid状态
    ...
    private void parseMax(byte[] bytes, int count) {
        switch (count) {
            case 0:
                RegisterParseUtil.parse03Hold0T44OldInv(mMaxData, bytes);
                break;
            case 1:
                RegisterParseUtil.parse03Hold45T89OldInv(mMaxData, bytes);
                break;
            case 2:
                RegisterParseUtil.parseHold180T224OldInv(mMaxData, bytes);
                break;
            case 3:
                RegisterParseUtil.parse04Input0T44OldInv(mMaxData, bytes);
                break;
            case 4:
                RegisterParseUtil.parse04Input45T89OldInv(mMaxData, bytes);
                break;
            case 5:
                RegisterParseUtil.parse04Input90T134OldInv(mMaxData, bytes);
                break;
//            case 4:
...
//                RegisterParseUtil.parseMax2OldInv(mMaxData,bytes);
//                break;
//            case 1:
...
//                break;
    ...
    private void sendMsg(SocketClientUtil clientUtil, int[] sends) {
        if (clientUtil != null) {
            clientUtil.sendMsg(ModbusUtil.sendMsgOldInv(sends[0], sends[1], sends[2]));
        } else {
            connectServer();
        }
    }
...
//            showOrHideRecyclerParams(mC6Adapter,t6H2IvStatus);
            ...
            jumpMaxSet(OldInvChartEnergyActivity.class,tvEnergyStr.getText().toString());
        }
//        else if (v == cvPower){
...
//        }
            ...
            if (!TextUtils.isEmpty(tvErrH1.getText().toString())) {
                stopRefresh();
                Intent intent = new Intent(mContext, OldInvWarningActivity.class);
            ...
            switch (what) {
                ...
                case SocketClientUtil.SOCKET_SEND:
                    if (autoCount < autoFun.length) {
                        byte[] sendBytesR = SocketClientUtil.sendMsgToServerOldInv(mClientUtilRead, autoFun[autoCount]);
    ...
    Handler mHandlerReadType= new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (what) {
                //发送信息
                case SocketClientUtil.SOCKET_SEND:
                    byte[] sendBytesR = SocketClientUtil.sendMsgToServerOldInv(mClientUtilReadType, deviceTypeFun);
                    LogUtil.i("发送读取：" + SocketClientUtil.bytesToHexString(sendBytesR));
                    break;
                //接收字节数组
                case SOCKET_RECEIVE_BYTES:
...
//                            toast(R.string.all_success);
    ...
    public void onItemClick(final BaseQuickAdapter adapter, View view, final int position) {
        if (adapter == mControlAdapter){
            ...
            switch (position){
                case 0:
                    clazz = OldInvFastConfigActivity.class;
                    break;
                case 1:
                    clazz = OldInvConfigActivity.class;
                    break;
                case 2:
                    clazz = OldInvParamsActivity.class;
                    break;
                case 3:
                    clazz = OldInvAdvanceSetActivity.class;
                    break;
                default:
                    clazz = null;
                    break;
         ...
         /*   if (GlobalConstant.MAX_NEED_PWD_FALSE.equals(SharedPreferencesUnit.getInstance(mContext).get(GlobalConstant.MAX_NEED_PWD))){
                ...
                //判断有无设置密码
                MaxUtil.getControlMaxPwd(this, new OnHandlerStrListener() {
                    @Override
                    public void handlerDealStr(String result) {
                        if ("-1".equals(result)) {
                            //验证密码
                            if (promptMaxPwd) {
                                MaxUtil.showSetMaxPwd(ToolMainOldInvActivity.this, new OnHandlerStrListener() {
                                    @Override
                                    public void handlerDealStr(String result) {
                                        if ("1".equals(result)) {
                                            //密码验证成功：跳转界面：不在提示
