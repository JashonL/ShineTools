...
public class  MixToolMainActivity extends DemoBase implements View.OnClickListener, BaseQuickAdapter.OnItemClickListener {
...
//    };
    ...
    int[][] funs = {{3, 0, 124},{3, 125, 249},{4,0,124},{4,1000,1124}};
    //读机器型号功能码
...
//    int[][] autoFun = {{4,0,124},{4,125,249},{4,3000,3124}};
    int[][] autoFun = {{4,0,124},{4,1000,1124}};
    int autoCount = 0;
    //开启自动刷新
    ...
    private String[] mixPriority;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_mix_tool_main);
...
//                "设置配置","参数设置","智能检测","高级设置"
    ...
    private void initContent1() {
        ...
        TextView tvTitle1Head2 = title1Head2.findViewById(R.id.tvHeadTitle);
        tvTitle1Head2.setText(getString(R.string.m403PV电压) + "/" + getString(R.string.m320功率_ios));
...
//        header1 = LayoutInflater.from(this).inflate(R.layout.header_maxmain2, (ViewGroup) mRecyclerView.getParent(), false);
        header2 = LayoutInflater.from(this).inflate(R.layout.header_mix_tool_main_head2, (ViewGroup) mRecyclerView.getParent(), false);
...
//        }
            ...
            jumpMaxSet(MixLiwangActivity.class,"");
        }
        else if (v == tvTitleBdc) {
            EventBus.getDefault().postSticky(mMaxData);
            jumpMaxSet(MixBdcActivity.class,"");
        }
        else if (v == tvTitleBattry) {
            EventBus.getDefault().postSticky(mMaxData);
            jumpMaxSet(MixBatteryActivity.class,"");
        }
//        else if (v == cvPower){
...
//        datasAbout[5] = deviceBeen.getFirmVersionIn();
    ...
    public void onItemClick(final BaseQuickAdapter adapter, View view, final int position) {
        if (adapter == mControlAdapter){
            ...
            switch (position){
                case 0:
                    clazz = MIXToolConfigActivity.class;
                    break;
                case 1:
                    clazz = MixToolParamActivity.class;
                    break;
                case 2:
...
//                    break;
        ...
        /*    if (GlobalConstant.MAX_NEED_PWD_FALSE.equals(SharedPreferencesUnit.getInstance(mContext).get(GlobalConstant.MAX_NEED_PWD))){
                ...
                //判断有无设置密码
                MaxUtil.getControlMaxPwd(MixToolMainActivity.this, new OnHandlerStrListener() {
                    @Override
                    public void handlerDealStr(String result) {
                        if ("-1".equals(result)) {
                            //验证密码
                            if (promptMaxPwd) {
                                MaxUtil.showSetMaxPwd(MixToolMainActivity.this, new OnHandlerStrListener() {
                                    @Override
                                    public void handlerDealStr(String result) {
                                        if ("1".equals(result)) {
                                            //密码验证成功：跳转界面：不在提示
