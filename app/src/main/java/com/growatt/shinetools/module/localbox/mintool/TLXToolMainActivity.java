...
//    int[][] funs = {{3, 0, 124},{3, 125, 249}, {4, 0, 99}, {4, 100, 199},{4,3000,3124}};
    int[][] funs = {{3, 0, 124},{3, 125, 249},{4,3000,3124}};
    //读机器型号功能码
...
//        header1 = LayoutInflater.from(this).inflate(R.layout.header_maxmain2, (ViewGroup) mRecyclerView.getParent(), false);
        header2 = LayoutInflater.from(this).inflate(R.layout.header_tlx_tool_main_head2, (ViewGroup) mRecyclerView.getParent(), false);
...
//                    Log.i("receive" ,recMsg);
                ...
                case SOCKET_RECEIVE_BYTES:
                    ...
                    try {
                        ...
                        if (isCheck) {
                            LogUtil.d("校验过的数据"+ CommenUtils.bytesToHexString(bytes));
                            //接收正确，开始解析
...
//        int deratMode = deviceBeen.getDerateMode();
//        if (deratMode >=0 && deratMode <=9){
...
//        datasAbout[5] = deviceBeen.getFirmVersionIn();
    ...
    public void onItemClick(final BaseQuickAdapter adapter, View view, final int position) {
       ...
       /*     if (GlobalConstant.MAX_NEED_PWD_FALSE.equals(SharedPreferencesUnit.getInstance(mContext).get(GlobalConstant.MAX_NEED_PWD))){
                ...
                //判断有无设置密码
                MaxUtil.getControlMaxPwd(TLXToolMainActivity.this, new OnHandlerStrListener() {
                    @Override
                    public void handlerDealStr(String result) {
                        if ("-1".equals(result)) {
                            //验证密码
                            if (promptMaxPwd) {
                                MaxUtil.showSetMaxPwd(TLXToolMainActivity.this, new OnHandlerStrListener() {
                                    @Override
                                    public void handlerDealStr(String result) {
                                        if ("1".equals(result)) {
                                            //密码验证成功：跳转界面：不在提示
...
//                                            OssUtils.circlerDialog(MaxMain2Activity.this, getString(R.string.m密码错误), -1, false);
   ...
   private void goToFastSet(){
       stopRefresh();
       Intent intent = new Intent(mContext, TlxFastConfigActivity.class);
