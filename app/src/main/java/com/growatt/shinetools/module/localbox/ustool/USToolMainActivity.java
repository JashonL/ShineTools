...
//    int[][] autoFun = {{4,0,124},{4,125,249},{4,3000,3124}};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ustool_main);
...
//        tvAdvanceSet = (TextView) header1.findViewById(R.id.tvAdvanceSet);
        ...
        llAdvanSet = header1.findViewById(R.id.llAdvanSet);
        ...
        String[] mConTitles = new String[]{
                ...
                getString(R.string.m充放电管理),
                getString(R.string.m285智能检测), getString(R.string.快速设置)
//                "设置配置","参数设置","智能检测","高级设置"
        };
        int[] mConImgId = new int[]{
                R.drawable.max_set, R.drawable.max_parameter, R.drawable.tlxh_control_manager,R.drawable.max_intelligent_icon, R.drawable.qiuckset
        };
...
//        header1 = LayoutInflater.from(this).inflate(R.layout.header_maxmain2, (ViewGroup) mRecyclerView.getParent(), false);
        header2 = LayoutInflater.from(this).inflate(R.layout.header_tlxh_us_tool_main_head2, (ViewGroup) mRecyclerView.getParent(), false);
...
//        tvTotalEH1.setText(mMaxData.getTotalEnergy() + "kWh");
        ...
        String[] datasAbout = new String[7];
...
//            }
        ...
        }else if (v == llAdvanSet){
            onItemClick(mControlAdapter,null,5);
        }
...
//        datasAbout[5] = deviceBeen.getFirmVersionIn();
    ...
    public void onItemClick(final BaseQuickAdapter adapter, View view, final int position) {
        if (adapter == mControlAdapter){
            ...
            switch (position){
                case 0:
                    clazz = USToolConfigActivity.class;
                    break;
                case 1:
                    clazz = USToolParamActivity.class;
                    break;
                case 2:
                    clazz = USToolChargeManagerActivity.class;
                    break;
                case 3:
            ...
            if (position==0||position==2){//设置配置和充放电管理开放密码
...
/*
            if (GlobalConstant.MAX_NEED_PWD_FALSE.equals(SharedPreferencesUnit.getInstance(mContext).get(GlobalConstant.MAX_NEED_PWD))){
                ...
                //判断有无设置密码
                MaxUtil.getControlMaxPwd(USToolMainActivity.this, new OnHandlerStrListener() {
                    @Override
                    public void handlerDealStr(String result) {
                        if ("-1".equals(result)) {
                            //验证密码
                            if (promptMaxPwd) {
                                MaxUtil.showSetMaxPwd(USToolMainActivity.this, new OnHandlerStrListener() {
                                    @Override
                                    public void handlerDealStr(String result) {
                                        if ("1".equals(result)) {
                                            //密码验证成功：跳转界面：不在提示
