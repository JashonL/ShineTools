 ...
 * 1.智能检测功能，故障录波是200条
 */
public class MaxXMainActivity extends BaseActivity implements View.OnClickListener, BaseQuickAdapter.OnItemClickListener, Toolbar.OnMenuItemClickListener{
...
//    private TextView tvAdvanceSet;
    ...
    protected int getContentView() {
        return R.layout.activity_max_x;
    }
...
//        }
            ...
            if (!TextUtils.isEmpty(tvErrH1.getText().toString())) {
               ...
               ActivityUtils.startActivity(MaxXMainActivity.this,intent,false);
            } else {
                toast(errNode);
            }
        }
    ...
    private void jumpMaxSet(Class clazz, String title) {
        ...
        intent.putExtra(DeviceConstant.KEY_DEVICE_TYPE,DeviceConstant.MAX_MAX_X_INDEX);
    ...
    public void onItemClick(final BaseQuickAdapter adapter, View view, final int position) {
       ...
       /*
            if (GlobalConstant.MAX_NEED_PWD_FALSE.equals(SharedPreferencesUnit.getInstance(mContext).get(GlobalConstant.MAX_NEED_PWD))){
                ...
                //判断有无设置密码
                MaxUtil.getControlMaxPwd(MaxXMainActivity.this, new OnHandlerStrListener() {
                    @Override
                    public void handlerDealStr(String result) {
                        if ("-1".equals(result)) {
                            //验证密码
                            if (promptMaxPwd) {
                                MaxUtil.showSetMaxPwd(MaxXMainActivity.this, new OnHandlerStrListener() {
                                    @Override
                                    public void handlerDealStr(String result) {
                                        if ("1".equals(result)) {
                                            //密码验证成功：跳转界面：不在提示
...
//                                            OssUtils.circlerDialog(MaxMain2Activity.this, getString(R.string.m密码错误), -1, false);
                            ...
                            if (clazz != null){
                                stopRefresh();
                                Intent intent = new Intent(MaxXMainActivity.this, clazz);
