...
import com.growatt.shinetools.adapter.OldInvErrorHisAdapter;
...
public class OldInvErrorHistoryActivity extends DemoBase {
    ...
    private OldInvErrorHisAdapter mAdapter;
    int[][] autoFun = {{4,90,114}};
    int autoCount = 0;
    //解析数据存储
    ...
    private void initRecyclerView() {
        ...
        mList = new ArrayList<>();
        mAdapter = new OldInvErrorHisAdapter(R.layout.item_max_error_history,mList);
    ...
    private void parseMax(byte[] bytes, int count) {
        List<MaxErrorBean> newList = RegisterParseUtil.parseOldInvHisErrInput90T114(bytes);
        switch (count) {
            case 0:
                parseList.clear();
