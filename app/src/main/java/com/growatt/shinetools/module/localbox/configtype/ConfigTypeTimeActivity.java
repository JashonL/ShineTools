 ...
 * 单个寄存器选择设置
 */
public class ConfigTypeTimeActivity extends BaseActivity implements Toolbar.OnMenuItemClickListener{
    String readStr;
    ...
    protected int getContentView() {
        return R.layout.activity_config_type_time;
    }
                    ...
                    if (funsSet != null) {
                        ...
                        for (int i = 0, len = funsSet[mType].length; i < len; i++) {
                            if (funsSet[mType][i][2] != -1) {
                                isWriteFinish = false;
                                sendBytes = SocketClientUtil.sendMsgToServer(mClientUtilW, funsSet[mType][i]);
                ...
                case SocketClientUtil.SOCKET_RECEIVE_BYTES:
                    ...
                    try {
                        ...
                        if (isCheck) {
                            ...
                            tvContent1.setText(readStr + ":" + timeStr);
