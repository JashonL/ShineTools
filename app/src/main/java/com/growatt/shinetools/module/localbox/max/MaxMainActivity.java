...
//    private TextView tvAdvanceSet;
    ...
    int[][] funs = {{3, 0, 124}, {4, 0, 99}, {4, 100, 199}};
    //读机器型号功能码
    ...
    protected int getContentView() {
        return R.layout.activity_max_main;
    }
    ...
    protected void initViews() {
        ...
        if (promptWifi) {
            ...
            if (!CommenUtils.isWifi(this)) {
                MaxUtil.showJumpWifiSet(this, getString(R.string.android_key610), getString(R.string.android_key612));
            }
        }
    }
    ...
    private void initString() {
        ...
        noteStartStr = getString(R.string.android_key589);
        noteStopStr = getString(R.string.android_key615);
        errNode = getString(R.string.android_key636);
        pidStatusStrs = new String[]{
                "", getString(R.string.android_key251), getString(R.string.android_key2324), getString(R.string.android_key316)
        };
        statusTitles = new String[]{
                getString(R.string.android_key251), getString(R.string.android_key2324),
                getString(R.string.android_key501), getString(R.string.android_key2044)
        };
        c1Title2 = new String[]{
                String.format("%s(V)", getString(R.string.android_key538)),
                String.format("%s(A)", getString(R.string.android_key698))
        };
        c2Title2 = new String[]{
                String.format("%s(V)", getString(R.string.android_key538)),
                String.format("%s(A)", getString(R.string.android_key698))
        ...
        c3Title2 = new String[]{
                String.format("%s(V)", getString(R.string.android_key538)),
                String.format("%s(Hz)", getString(R.string.android_key705)),
                String.format("%s(A)", getString(R.string.android_key698)),
                String.format("%s(W)", getString(R.string.android_key1164)),
                "PF"
        };
        c34Title2 = new String[]{
                String.format("%s(A)", getString(R.string.android_key1277)),
                String.format("%s(Var)", getString(R.string.android_key1279)),
                String.format("%s(A)", getString(R.string.android_key1281)),
                String.format("%s(Var)", getString(R.string.android_key1283)),
                String.format("%s(A)", getString(R.string.android_key1285)),
                getString(R.string.android_key1287),
        };
        c4Title2 = new String[]{
                String.format("%s(V)", getString(R.string.android_key538)),
                String.format("%s(mA)", getString(R.string.android_key698))
        };
        c6Title1 = new String[]{
                getString(R.string.android_key667), getString(R.string.android_key669),
                "ISO",
                getString(R.string.android_key671), getString(R.string.android_key673),
                getString(R.string.android_key675),
                "+Bus", "-Bus",
                getString(R.string.android_key680), getString(R.string.android_key682)
        };
        c5Title1 = new String[]{
                getString(R.string.android_key684), getString(R.string.android_key686),
                getString(R.string.android_key1094), getString(R.string.android_key688),
//                getString(R.string.m315固件外部版本),  getString(R.string.m316固件内部版本)
                getString(R.string.android_key1090), getString(R.string.android_key1100)
        };
    }

    private void initHeaderView() {   //头部
        initToobar(toolbar);
        toolbar.inflateMenu(R.menu.main_menu);
        menuItem = toolbar.getMenu().findItem(R.id.action_mode_setting);
        menuItem.setTitle(R.string.m268自动刷新);
...
//        tvAdvanceSet = (TextView) header1.findViewById(R.id.tvAdvanceSet);
        ...
        String[] mConTitles = new String[]{
                ...
                getString(R.string.android_key625), getString(R.string.android_key382)
//                "设置配置","参数设置","智能检测","高级设置"
        };
...
//        adapter.setNewData(newList);
    ...
    private void initContent34() {
        ...
        content34Head2.setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getResources().getDimensionPixelSize(R.dimen.dp_130)));
...
//                        }
                ...
                case 101:
                    ...
                    break;
                default:
                    BtnDelayUtil.dealMaxBtn(this, what, 2500, MaxMainActivity.this, toolbar);
                    break;
            }
...
//        Mydialog.Dismiss();
    ...
    private void refreshUI() {
        ...
        if (errCode >= 200 || warmCode >= 200) {
            ...
            if (errCode == 0) {
                errCodeStr = getString(R.string.android_key770);
            } else {
                errCode += 99;
//                errCodeStr = String.format("%d(%02d)",errCode,errCodeSecond<100?errCodeSecond:errCodeSecond%100);
            ...
            if (warmCode == 0) {
                warnCodeStr = getString(R.string.android_key772);
            } else {
                warmCode += 99;
//                warnCodeStr = String.format("%d(%02d)",warmCode,warmCodeSecond<100?warmCodeSecond:warmCodeSecond%100);
        ...
        if (status >= 0 && status < statusTitles.length) {
            ...
            statusStr = getString(R.string.android_key1195) + status;
        }
...
//        }
            ...
            if (!TextUtils.isEmpty(tvErrH1.getText().toString())) {
                stopRefresh();
                Intent intent = new Intent(this, TLXWarningActivity.class);
...
//                            toast(R.string.all_success);
    ...
    public void onItemClick(final BaseQuickAdapter adapter, View view, final int position) {
       ...
       /*     if (GlobalConstant.MAX_NEED_PWD_FALSE.equals(SharedPreferencesUnit.getInstance(this).get(GlobalConstant.MAX_NEED_PWD))) {
                if (clazz != null) {
                    jumpMaxSet(clazz, title);
                } else if (position == 0) {
                    goToFastSet();
                ...
                //判断有无设置密码
                MaxUtil.getControlMaxPwd(MaxMainActivity.this, new OnHandlerStrListener() {
                    @Override
                    public void handlerDealStr(String result) {
                        if ("-1".equals(result)) {
                            //验证密码
                            if (promptMaxPwd) {
                                MaxUtil.showSetMaxPwd(MaxMainActivity.this, new OnHandlerStrListener() {
                                    @Override
                                    public void handlerDealStr(String result) {
                                        if ("1".equals(result)) {
                                            //密码验证成功：跳转界面：不在提示
...
//                                            OssUtils.circlerDialog(MaxMain2Activity.this, getString(R.string.m密码错误), -1, false);
                                            new CircleDialog.Builder()
                                                    .setWidth(0.7f)
                                                    .setTitle(getString(R.string.android_key2263))
                                                    .setText(getString(R.string.android_key3013))
                                                    .setPositive(getString(R.string.android_key619), new View.OnClickListener() {
                                                        @Override
                                                        public void onClick(View v) {
                            ...
                            if (clazz != null) {
                                stopRefresh();
                                Intent intent = new Intent(MaxMainActivity.this, clazz);
                                intent.putExtra("type", result);
                                ActivityUtils.startActivity(MaxMainActivity.this,intent,false);
                            }
                        }
                    }
                });
    ...
    private void goToFastSet() {
        stopRefresh();
        Intent intent = new Intent(this, TlxFastConfigActivity.class);
